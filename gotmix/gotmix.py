...
class gotmix(commands.Cog):

    """GOT Comics with an 80s twist. (Adapted from Weeedcog)"""

    def __init__(self, red: Red):
        self.bot = red
        # darkforge fingerprint
        config_id = 112353211
        ...
        self.default_config_guild = {
            ...
            "comic_text": '**I  :heart:  the  80\'s !**',
            "font": 'BadaboomBB_Reg.ttf'
        }
    ...
    @commands.group()
    async def gotmix(self, ctx: commands.Context):
        """This is the primary cog group. Try ``"""
        pass

    @gotmix.group(name='set')
    @checks.mod()
    async def set(self, ctx: commands.Context):
        """Configure various Cog settings."""
        pass
    ...
    async def background_image(self, ctx, filename: str = None):
        ...
        if not filename:
            ...
            if filename in files:
                ...
                await ctx.send("The background_image is set to \"random\"")
    ...
    async def max_messages(self, ctx: commands.Context, max: int = None):
        ...
        if not max:
            ...
            await ctx.send("Setting this higher than 10 will result in files too big to post.")
        elif max not in range(1, 9):
    ...
    def _sanitize_emojinames(text):
        regex = re.compile(r"(?:<a?)(\:[0-9aA-zZ\u0000-\u007F]+\:)(?:[0-9]+>)")
        result = re.sub(
                    regex,
                    lambda m: m.group(1),
                    text
    ...
    async def _messages_to_comicdata(self, messages: List[discord.Message]):
        ...
        print(f"[GOTMIX] Comic data generated! Data follows:\n{comic}")
        return comic
    ...
    @gotmix.command()
    async def get(self, ctx: commands.Context, count: int, message_id: int = None):
        """Make Comic with X amt of Posts"""
        ...
        all_backgrounds = listdir(f"{self.datapath}/background")
        if background_image not in all_backgrounds:
            shuffle(all_backgrounds)
            background_image = all_backgrounds[0]
        ...
        for panel_count in range(0, len(comic)):
            ...
            draw.multiline_text((left_buffer+1, top_buffer+1), left_text, font=font, fill="black")
            draw.multiline_text((left_buffer-1, top_buffer-1), left_text, font=font, fill="black")
            ...
            draw.multiline_text((left_buffer+1, top_buffer+1), right_text, font=font, fill="black")
            draw.multiline_text((left_buffer-1, top_buffer-1), right_text, font=font, fill="black")
